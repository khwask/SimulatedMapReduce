...
public class RelativityCalculationReducer extends Reducer<String, String, NullWritable, String> {
	...
	public void reduce(String key, Iterable<String> values, Context context) {
		...
		String goodsName = key.substring(0, key.length() - 2);;
		...
		while (iterator.hasNext()) {
			String[] goodsNameAndNum = iterator.next().split(",");
			String pairGoodsName = goodsNameAndNum[0] /* 関連度を計算する商品Yの名前を設定 */;
			double relativity = Double.parseDouble(goodsNameAndNum[1]) / denominator /* 関連度を計算 */;
